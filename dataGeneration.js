...
let lastBobbinChangeTime = Date.now();
let thisMonthProduction = 0;
let lastProductionUpdateTime = Date.now();
let actualProduction = 0;
let previousActSpeed = 25.0;
...
function generateMachineTestData() {
  ...
  // Machine should not go to 'offline' if breakdown occurs
  if (!activeBreakdown && now - lastMcStatusChangeTime > randomMcStatusDuration) {
    currentMcStatus = Math.random() < 0.7 ? 1 : 0;
    lastMcStatusChangeTime = now;
    randomMcStatusDuration = Math.floor(Math.random() * (maxDuration - minDuration + 1)) + minDuration;
  }
  ...
  let actSpeed = 0;
  let activeTime = 0;
  const activeDuration = 12 * 60 * 60 * 1000;

  if (currentMcStatus === 1) {
    if (now - lastBobbinChangeTime < activeDuration) {
  ...
  if (now - blockChangeTime > 10 * 60 * 1000) {
    activeBlock = Math.floor(Math.random() * 7) + 1;
    blockChangeTime = now;
    setTimeout(() => {
      activeBlock = null;
    }, 2 * 60 * 1000);
  ...
  const blocks = Array.from({ length: 7 }, (_, i) => i + 1).reduce((acc, num) => {
    ...
    acc[`Block${num} Single/Multi`] = false;
    acc[`Block${num} WBS`] = false;
    acc[`Block${num} Wire Size`] = +(2.5 + Math.random() * 2.5).toFixed(2);
    return acc;
  }, {});
  ...
  // Ensure MC_STATUS remains 1 during breakdowns
  if (activeBreakdown) {
    e_dt_status[activeBreakdown] = 1;
    actSpeed = 0; // Machine still runs but speed is 0
    currentMcStatus = 1; // Ensuring machine remains ON during breakdown
  } else {
    if (currentMcStatus === 1 && Math.random() < 0.01) {
      activeBreakdown = e_dt_keys[Math.floor(Math.random() * e_dt_keys.length)];
      breakdownStartTime = now;
    }
  }
  ...
  if (currentMcStatus === 1 && actSpeed > 0) {
    thisMonthProduction += 5 * actSpeed;
  } else if (!isNewMonth && now - lastProductionUpdateTime > 60000) {
    lastProductionUpdateTime = now;
    thisMonthProduction += 5 * actSpeed;
...
client.on('connect', () => {
  ...
  setInterval(() => {
    ...
    if (toggle) {
      data = generateMachineTestData();
      actualProduction = data['This Month Production'];
    }
    toggle = !toggle;
